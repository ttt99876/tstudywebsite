...
                        **权限列表**：用于列出授权使用的各种数据库操作，以逗号进行分隔，如“select,insert,update”等。使用“all”表示所有权限，可授权执行任何操作。

                        **数据库名.表名**：用于指定授权操作的数据库和表的名称，其中可以使用通配符“*.*”表示全部。例如，使用“db3.*”表示授权操作的对象为 db3数据库中的所有表，有危险。

                        **用户名@来源地址**：用于指定用户名称和允许访问的客户机地址，即谁能连接、能从哪里连接。来源地址可以是域名、IP 地址，还可以使用“%”通配符，表示某个区域或网段内的所有地址，如“%.my.com”、“192.168.72.%”等。

                        **IDENTIFIED BY**：用于设置用户连接数据库时所使用的密码字符串。在新建用户时，若省略“IDENTIFIED BY”部分， 则用户的密码将为空。

```js
// -- GRANT 权限列表 ON 数据库名.表名 TO '用户名'@'主机地址' [IDENTIFIED BY '密码'];
GRANT select ON db3.account TO 'ttt1'@'%';
// -- 测试:使用db3，此时可以使用
use db3;
// 此时可以查询account表,但是其他操作不能进行，因为其他操作没有被授予权限
select * from account;
// 有危险，不建议这样干
GRANT all ON *.* TO 'ttt1'@'%';
```
        3、撤销权限

                **语法**：REVOKE 权限列表* ON 数据库名.表名 FROM '用户'@'来源地址'; #撤销所有权限

```js
// 撤销所有权限
revoke all on *.* from 'ttt1'@'%';
```
