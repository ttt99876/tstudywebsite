...
        通过class属性进行元素的获取，可用于对多个元素进行相同的样式设置。css样式写法：.类名{}
```css
.div-cls{
    color: red;
}
...
        通过id属性给元素一个唯一标识符（多个相同的id不会报错，但是不建议这样干，建议id唯一）。css样式写法：#id{}
```css
...
         使用font-style设置，常用于规定斜体文本。该属性有三个值：
